<template>
  <div v-if="store.state.user"  class=" min-h-full  md:h-screen bg-gray-100">
      ...
      <div v-show="showNav" class="fixed z-10 top-0 bg-gray-100 pt-14">
        <div class="flex flex-col justify-center  rounded-br-lg  p-2 w-72  divide-y-2 text-center z-0">
          ...
          <div v-if="store.state.user.role == 'Teacher'"
            class="courseNavigation flex-col text-base font-medium gap-3 p-2 hidden md:flex">
    ...
    <button @click="store.commit('SHOW_NAV')"
      class="fixed top-1/2 left-0 rounded-tr-lg h-[70px] w-[30px] transition-all duration-300 rounded-br-lg bg-slate-100 z-10"
